...
class MockSearchFoodByIngredientsUseCase extends Mock implements SearchFoodByIngredientsUseCase {
  ...
  Future<Either<Failure, List<FoodItem>>> call(SearchFoodByIngredientsParams params) async {
       return super.noSuchMethod(Invocation.method(#call, [params])) ?? Future.value(const Right([]));
   }
}
...
class MockCheckDataExistenceUseCase extends Mock implements CheckDataExistenceUseCase {
  @override
  Future<Either<Failure, bool>> call(NoParams params) async => super.noSuchMethod(Invocation.method(#call, [params])) ?? Future.value(const Right(false));
}
...
class MockGetStorageInfoUseCase extends Mock implements GetStorageInfoUseCase {
   @override
  Future<Either<Failure, StorageInfo>> call(NoParams params) async => super.noSuchMethod(Invocation.method(#call, [params])) ?? Future.value(const Right(StorageInfo(count: 0, sizeBytes: 0)));
}
...
void main() {
  ...
  setUpAll(() {
    ...
    if (getIt.isRegistered<SearchFoodByIngredientsUseCase>()) getIt.unregister<SearchFoodByIngredientsUseCase>();
    if (getIt.isRegistered<GetSuggestedIngredientsUseCase>()) getIt.unregister<GetSuggestedIngredientsUseCase>();

    getIt.registerLazySingleton<SearchFoodByIngredientsUseCase>(() => MockSearchFoodByIngredientsUseCase());
    getIt.registerLazySingleton<GetSuggestedIngredientsUseCase>(() => MockGetSuggestedIngredientsUseCase());
  });
  ...
  setUp(() {
    ...
    final getIt = GetIt.instance;
    if (getIt.isRegistered<IngredientSearchCubit>()) getIt.unregister<IngredientSearchCubit>();
  ...
  testWidgets('Desktop: Ingredient selection adds chip and clears text', (WidgetTester tester) async {
    // Arrange
    ...
    // 2. Enter '철'
    final textField = find.byType(TextField); // Should find one in IngredientSearchTab
    ...
    final EditableText editableText = tester.widget(find.byType(EditableText).first);
